...
- Dependency versions/coordinates used by the build scripts are centralized in the shared version catalog [gradle/libs.versions.toml](gradle/libs.versions.toml), consumed by both the root build and `buildSrc` (registered in [buildSrc/settings.gradle.kts](buildSrc/settings.gradle.kts)). Versions that also belong in `com.huanshankeji.CommonVersions` (Kotlin, Compose Multiplatform, kotlinx-benchmark) are taken from `CommonVersions` / `commonGradleClasspathDependencies` instead of being duplicated in the catalog; full replacement of `common-gradle-dependencies` remains #9.
