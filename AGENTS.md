...
- Dependency versions/coordinates used by the build scripts are centralized in the shared version catalog [gradle/libs.versions.toml](gradle/libs.versions.toml), consumed by both the root build and `buildSrc` (registered in [buildSrc/settings.gradle.kts](buildSrc/settings.gradle.kts)). Overlapping values used by `com.huanshankeji.CommonVersions` (`kotlin`, `composeMultiplatform`, `kotlinx-benchmark`) are generated into `GeneratedVersions` from that catalog; full replacement of `common-gradle-dependencies` remains #9.
