...
**Toolchain:** JDK **17** (toolchain; CI uses Temurin 17 — see [.github/workflows/kotlin-jvm-ci.yml](.github/workflows/kotlin-jvm-ci.yml)). Gradle version: [gradle/wrapper/gradle-wrapper.properties](gradle/wrapper/gradle-wrapper.properties). Build-logic Kotlin version: [gradle/libs.versions.toml](gradle/libs.versions.toml) (`[versions] kotlin`); see [README.md](README.md) ("Build-logic Kotlin vs Gradle's embedded Kotlin"). Published plugin dependency versions: [VersionsAndDependencies.kt](buildSrc/src/main/kotlin/VersionsAndDependencies.kt) and `com.huanshankeji.CommonVersions`.
...
- There are no longer cross-version bootstrapping dependencies on released artifacts of this repository (#54):
    ...
    - Each source-linking subproject applies the Kotlin plugin via `kotlin("jvm")` (no version; the version is registered in [buildSrc/build.gradle.kts](buildSrc/build.gradle.kts) with `alias(libs.plugins.kotlin.jvmWithExplicitVersion) apply false`) and configures its source directory before applying `kotlin-dsl` imperatively, working around [gradle/gradle#21052](https://github.com/gradle/gradle/issues/21052).
    - Build-logic Kotlin is pinned via the catalog `[plugins] kotlin-jvmWithExplicitVersion` alias, not by repeating `version.ref = "kotlin"` on `org.jetbrains.kotlin:*` [libraries] entries (those omit a version and align via the Kotlin Gradle plugin BOM). Do not rely on `pluginManagement` in `buildSrc/settings.gradle.kts` alone — see [README.md](README.md). Do not add the same `apply false` alias to the root [build.gradle.kts](build.gradle.kts): `buildSrc` already supplies Kotlin plugins on the classpath.
