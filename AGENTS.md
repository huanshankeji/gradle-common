...
- Dependency versions/coordinates used by the build scripts are centralized in the shared version catalog [gradle/libs.versions.toml](gradle/libs.versions.toml), consumed by both the root build and `buildSrc` (registered in [buildSrc/settings.gradle.kts](buildSrc/settings.gradle.kts)). Overlaps with `com.huanshankeji.CommonVersions` stay hand-synced: Gradle's [`version-catalog`](https://docs.gradle.org/current/userguide/version_catalogs.html#sec:version-catalog-plugin) plugin publishes a TOML for settings-time `from()`, which does not feed published Kotlin default constants; unifying that is #9.
