...
2. `buildSrc` source-links the `common-gradle-dependencies` and `kotlin-common-gradle-plugins` sources (see `buildSrc/build.gradle.kts`) and the plugin modules depend on the `common-gradle-dependencies` project directly, so there are no longer cross-version bootstrapping dependencies on released artifacts of this repository, and there is no need to publish anything to Maven local before building.
